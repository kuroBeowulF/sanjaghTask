  ...
  return (
    <div className="border-b border-b-[#F5F5F5]">
      <Link href={`${serviceId}/${zoneId}`} className="text-black">
        <li className="p-3 cursor-pointer hover:bg-[#E5F1FF] list-none"> { serviceName } </li>
      </Link>
    </div>
  )
}
...
const SearchResults = ({ results, zoneId }) => {
  return (
    <div
      className="bg-white overflow-auto z-10 mt-12 top-0 max-h-72 absolute w-full border border-[#EAECED] rounded-b-md shadow-sm scrollbar-minimal">
      {results.map(({id, name}) =>
        <SearchItem
          key={id}
          zoneId={zoneId}
...
export const FullPageWrapper = ({ onBackButtonClick, children, searchType }) => {
  return (
    <div
      className="fixed top-0 bottom-0 left-0 right-0 bg-white z-[99999] cursor-default"
    >
      <SearchHeader onBackButtonClick={onBackButtonClick} searchType={searchType} />
...
const ServiceSearch = ({ zoneName, zoneId, searchType, setSearchType }) => {
  ...
  return (
    <FullPageWrapper onBackButtonClick={() => setSearchType("CLOSED")} searchType={searchType}>
...
const City = ({ zoneId, zoneName, setCurrentZone, setSearchType }) => {
  ...
  return (
    <div onClick={handleCitySelect} className="border-b border-b-gray-100 text-gray-500 py-3 px-2 cursor-pointer hover:bg-[#E5F1FF]">
      <span> { zoneName } </span>
    </div>
  )
}
...
const CitySearch = ({ searchType, setSearchType, setCurrentZone }) => {
  ...
  return (
    <FullPageWrapper onBackButtonClick={() => setSearchType("ServiceSearch")} searchType={searchType}>      
      <div className="flex items-center border rounded-lg border-gray-200 p-3">
        <HiOutlineLocationMarker
          className="ml-1 xsm:ml-2" size="1.2rem" color="#D3D4D5"
        />
        <input
          type="text"
...
const FullPageSearch = ({ containerClassName, zoneName, zoneId, setCurrentZone }) => {
